...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($_POST['delete'])) {
    ...
    // Formatear tiempo jugado para coincidir con la columna TEXT 'time_played' de la BBDD
    // Formato ejemplo: "1h 30m" o "45m"
    $parts = [];
    if ($hours > 0)
      $parts[] = "{$hours}h";
    if ($minutes > 0)
      $parts[] = "{$minutes}m";
    $timePlayed = empty($parts) ? "0m" : implode(" ", $parts);

    if ($id > 0) {
      $stmt = $db->prepare("UPDATE test_session SET tester=?, content=?, difficulty=?, score=?, time_played=?, comments=? WHERE id=?");
      $stmt->execute([$tester, $content, $diff, $score, $timePlayed, $comments, $id]);
      $mensaje = "Sesión actualizada.";
    } else {
      $stmt = $db->prepare("INSERT INTO test_session (tester, content, difficulty, score, time_played, comments) VALUES (?, ?, ?, ?, ?, ?)");
      $stmt->execute([$tester, $content, $diff, $score, $timePlayed, $comments]);
...
if (isset($_GET['edit'])) {
  ...
  if ($found) {
    ...
    // Parsear time_played (ej: "1h 30m" o "45m") para rellenar el formulario
    $item['time_hours'] = 0;
    $item['time_minutes'] = 0;

    if (!empty($item['time_played'])) {
      if (preg_match('/(\d+)h/', $item['time_played'], $m)) {
        $item['time_hours'] = (int) $m[1];
      }
      if (preg_match('/(\d+)m/', $item['time_played'], $m)) {
        $item['time_minutes'] = (int) $m[1];
      }
    }
...
<body class="<?= $claseCursor ?>">
  ...
  <main class="content">
    ...
    <?php if ($mensaje): ?>
      <div class="status success"><?= htmlspecialchars($mensaje) ?></div><?php endif; ?>
    <?php if ($error): ?>
      <div class="status err"><?= htmlspecialchars($error) ?></div><?php endif; ?>
      ...
      <div class="grid-col-table">
        <section class="panel" style="overflow-x:auto;">
          ...
          <div class="pagination">
            <?php if ($pagina > 1): ?><a href="?page=<?= $pagina - 1 ?>&search=<?= $search ?>"
                class="btn-wow secondary">Ant</a><?php endif; ?>
            <span style="padding:5px 10px; color:#aaa;">Pág <?= $pagina ?> de <?= $totalPaginas ?></span>
            <?php if ($pagina < $totalPaginas): ?><a href="?page=<?= $pagina + 1 ?>&search=<?= $search ?>"
