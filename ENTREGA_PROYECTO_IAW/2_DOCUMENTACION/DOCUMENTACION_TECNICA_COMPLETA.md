...
- **API Externa**: Blizzard Battle.net API con cURL nativo
...
- **Credenciales por defecto**:
  ...
  - Tester: *Debe ser creado por un administrador*

### 2. Gestión de Testers (Admin)
1. Navega a la sección **Testers** en el menú lateral.
2. **Crear**: Rellena el formulario "Nuevo Recluta" con Nombre, Rol (Tank/DPS/Healer) y Facción.
3. **Vincular con Blizzard**:
   - Ve a **Blizzard API**.
   - Selecciona un tester y escribe su Reino y Nombre de Personaje.
   - Pulsa "Vincular" y luego "Sincronizar" para traer datos oficiales (Nivel, Clase, Item Level).

### 3. Registro de Sesiones (Tester/Admin)
1. Ve a **Sesiones**.
2. Selecciona el **Tester** y el **Contenido** (Raid/Dungeon) realizado.
3. Introduce la **Puntuación** (0-100), **Dificultad** y **Tiempo Jugado** (Horas/Minutos).
   - *Nota: El tiempo se guardará automáticamente en formato legible (ej: "1h 30m").*
4. Añade comentarios sobre bugs o balanceo y pulsa "Registrar".

### 4. Generación de Reportes
1. Accede a **Reportes** (Solo Admin).
2. Visualiza las tablas de "Mejores Testers" y "Contenido Más Difícil".
3. **Exportar**:
   - **CSV**: Pulsa "Descargar CSV Completo" para usar en Excel.
   - **PDF**: Pulsa "Descargar PDF Profesional" para generar un informe oficial con diseño corporativo.

---

## ⚠️ Dificultades Encontradas y Soluciones

Durante el desarrollo de este proyecto, se superaron varios desafíos técnicos significativos:

### 1. Integración con Blizzard API (OAuth2)
- **Desafío**: La API de Blizzard requiere un flujo de autenticación OAuth2 "Client Credentials" antes de poder consultar datos.
- **Solución**: Se implementó una clase dedicada `BlizzardAPI` que gestiona automáticamente la obtención y renovación del token de acceso (Bearer Token). Se añadió cacheo local para minimizar peticiones y mejorar la velocidad de carga.

### 2. Formato de Tiempo en Base de Datos
- **Desafío**: SQLite no tiene tipo de dato nativo para intervalos de tiempo, y el formulario enviaba horas/minutos por separado, mientras que la base de datos esperaba un formato de texto único.
- **Solución**: Se desarrolló una lógica de transformación en el backend (`wow_sesiones.php`) que convierte los inputs numéricos a un string estandarizado (ej: "2h 15m") antes de guardar, y realiza el proceso inverso (parsing con expresiones regulares) al editar una sesión.

### 3. Generación de PDFs Complejos
- **Desafío**: Crear reportes PDF que mantuvieran la estética "World of Warcraft" (fuentes, colores, tablas) no era trivial con herramientas básicas.
- **Solución**: Se integró la librería **TCPDF** vía Composer. Se extendió la clase base para personalizar cabeceras y pies de página gráficos, y se maquetó el contenido utilizando celdas con colores de fondo y bordes específicos para emular el diseño del juego.

### 4. Persistencia y Seguridad de Sesiones
- **Desafío**: Mantener la sesión del usuario segura frente a ataques comunes.
- **Solución**: Se configuraron las cookies de sesión con flags `HttpOnly` y `SameSite`. Además, se implementó protección anti-CSRF (Cross-Site Request Forgery) generando tokens únicos por sesión que se verifican en cada petición POST.
